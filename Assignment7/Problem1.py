...
def main():
    ...
    error_bar_train = math.sqrt((8. / len(training_data)) * np.log((4 * ((2 * len(training_data))**3 + 1)) / 0.05))
    ...
    xspace = np.linspace(-250,50, 300)

    axarr[0].plot(xspace, formula(w_train, xspace), color='black', label='Training Separator')
    axarr[1].plot(xspace, formula(w_train, xspace), color='black', label='Testing Separator')
