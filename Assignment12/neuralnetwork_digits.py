...
def variable_rate_gradient_descent(eta, weights, current_error, previous_error, gradient):
    alpha = 0.03
    beta = -0.08

    if current_error < previous_error:
        weights = [w-eta*g for w, g in zip(weights, gradient)]
        eta += alpha*eta
    else:
        eta += beta*eta
...
def neural_network(data, num_layers, weights, num_iterations):
    N = float(len(data))
    errors = [0.] * (num_iterations+1)
    errors[0] = sys.maxsize

    # iteration 0 is dummy value - used for initial error
    for iteration in range(1, num_iterations+1):
        ...
        for data_point in data:
            ...
            errors[iteration] += float(((1/(4*N))*(x[-1]-data_point[1])**2)[0])

            # find gradientt for each layer
            for l in range(1, num_layers):
                G_l_point = np.dot(x[l-1], np.transpose(delta[l]))
                G[l] += (1/N)*G_l_point

        # perform variable rate gradient descent
        eta, weights = variable_rate_gradient_descent(eta, weights, \
                errors[iteration], errors[iteration-1], G)

    plt.plot(np.arange(num_iterations), errors[1:])
...
if __name__ == '__main__':
    ...
    opt_weights = neural_network(training_data, num_layers, weights, 50)
    plt.show()

    f, ax = plt.subplots(2)
    for point in testing_data:
        x = forward_propogation(point[0], num_layers, opt_weights)

        if point[1] == 1:
            ax[0].plot(point[0][1], point[0][2], 'bo')
        else:
            ax[0].plot(point[0][1], point[0][2], 'rx')
